...
from .additional_experts import DeployExpert, DbExpert, CiExpert, DocsExpert, CleanExpert, SetupExpert
...
class TaskAnalyzer:
    ...
    def get_task_recommendations(self) -> List[TaskRecommendation]:
        ...
        for expert in self.experts:
            ...
            new_recommendations = [
                ...
                if rec.task.full_name not in existing_tasks
            ]
    ...
    def validate_task_architecture(self) -> Dict[str, any]:
        ...
        no_description = [
            ...
            if not task.description and not task.hide
        ]
        if no_description:
    ...
    def find_redundant_tasks(self) -> List[Dict[str, any]]:
        ...
        graph = self.build_dependency_graph(tasks)
        for task in tasks:
            if (graph.in_degree(task.full_name) == 0 and 
                graph.out_degree(task.full_name) == 0 and
                not task.alias and 
                task.full_name != "default"):
                redundant.append({
                    "task": task.full_name,
