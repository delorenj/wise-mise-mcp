...
from .models import (
    TaskDefinition, TaskDomain, TaskComplexity, 
    ProjectStructure, TaskRecommendation
)
                    ...
                    if "build:prod" in scripts or "build:production" in scripts:
                        recommendations.append(TaskRecommendation(
                            task=TaskDefinition(
                                ...
                                run="npm run build:prod" if "build:prod" in scripts else "npm run build:production",
                                sources=["src/**/*", "package.json"],
                                outputs=["dist/", "build/"],
                    ...
                    if any(e2e in deps for e2e in ["playwright", "cypress", "@playwright/test"]):
                        recommendations.append(TaskRecommendation(
                            task=TaskDefinition(
                                ...
                                run="npx playwright test" if "playwright" in deps else "npx cypress run",
                                depends=["build"],
