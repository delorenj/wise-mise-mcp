...
from .models import (
    TaskDefinition, TaskDomain, TaskComplexity, MiseConfig, 
    TaskRecommendation, ProjectStructure
)
...
class TaskManager:
    ...
    def create_task_intelligently(
        ...
        if any(task.full_name == full_name for task in existing_tasks):
            return {
                ...
                "existing_task": next(task for task in existing_tasks if task.full_name == full_name)
            }
        
        # Create task definition
        task_def = self._create_task_definition(
            full_name, domain, task_description, complexity
        )
    ...
    def _determine_complexity(self, description: str) -> TaskComplexity:
        ...
        complex_indicators = [
            "multiple steps", "conditional", "if", "loop", "check", 
            "validate", "complex", "script", "sequence"
        ]
        
        moderate_indicators = [
            "build and", "test and", "multiple", "several", "both"
        ]
    ...
    def _generate_task_name(self, description: str, domain: TaskDomain) -> str:
        ...
        stop_words = {
            ...
            "for", "of", "with", "by", "run", "execute", "start", "create"
        }
    ...
    def remove_task(self, task_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if task_name in config.tasks:
                ...
                return {
                    "success": True,
                    "message": f"Removed TOML task '{task_name}'"
                }
